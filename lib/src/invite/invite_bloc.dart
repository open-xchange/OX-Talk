...
class InviteBloc extends Bloc<InviteEvent, InviteState> {
  ...
  Stream<InviteState> mapEventToState(InviteEvent event) async* {
    if (event is CreateInviteUrl) {
      ...
      yield* createChatWithInvite(event.inviteServiceResponse);
    }
  }
  ...
  Stream<InviteState> handleSharedInviteLink() async* {
    ...
    int startIndex = sharedLink.lastIndexOf('/') + 1;
    ...
    if (id.isNotEmpty) {
      ...
      if (valid) {
        InviteServiceResponse responseInviteService = _getInviteResponse(response);
        yield InviteStateSuccess(inviteServiceResponse: responseInviteService);
      } else {
        String errorMessage;
  ...
  Stream<InviteState> createChatWithInvite(InviteServiceResponse inviteServiceResponse) async* {
    ...
    String imageString = inviteServiceResponse.sender.image;
    ...
    if (imageString != null) {
      ...
      fileImage.evict();
      int startIndex = imageString.lastIndexOf(',') + 1;
      String image = imageString.substring(startIndex);
